...
test("generated flowcharts have renderable cached data", () => {
  Object.values(FLOWCHART_DATA).forEach((data) => {
    ...
    data.nodes.forEach((node) => {
      ...
      assert.equal(
        mismatch.kind,
        !mismatch.expectedSans.includes(generatedEdge.san)
          ? "notBest"
          : mismatch.expectedSans.length > 1
            ? "globalTie"
            : "implicit",
      );
    });
...
test("knight-bishop prepare renders one child for every failure path", () => {
  ...
  const rawNode = FLOWCHART_DATA.knightBishopPrepare.nodes.find(
    (candidate) => candidate.id === "n3",
  );
  assert.ok(rawNode);
  assert.equal(rawNode.fen, "8/4k3/4B3/4K3/8/6N1/8/8 w - - 0 1");
  const rawIdealMoves = Brain.getIdealEndgameWhiteMoves(rawNode.fen);
  assert.deepEqual(rawIdealMoves, ["Nf5+"]);

  const rawEdge = FLOWCHART_DATA.knightBishopPrepare.edges.find(
    (edge) => edge.id === rawNode.outgoingEdgeIds[0],
  );
  assert.ok(rawEdge);
  assert.equal(rawEdge.san, "Nh5");

  const renderedData = getRenderedFlowchartData(
    FLOWCHART_DATA.knightBishopPrepare,
  );
  assert.equal(
    renderedData.nodes.some((node) => node.id === "n5"),
    false,
  );
  ...
  assert.equal(renderedEdge.san, rawIdealMoves[0]);
  ...
  assert.ok(failurePathNodeIds.has("n3"));
  ...
  assert.ok(child);
  assert.equal(
    Brain.positionKey(child.fen),
    Brain.positionKey(Brain.getFen(rawNode.fen, rawIdealMoves[0])),
  );
...
test("knight-bishop rule 6 keeps the king in the middle 16 squares", () => {
  ...
  const fen = "8/8/8/3NK3/2k5/2B5/8/8 w - - 72 37";
  assert.deepEqual(Brain.getIdealEndgameWhiteMoves(fen), ["Ke4"]);
  assert.equal(
    Brain.getKnightAndBishopExplicitWhiteMoveReason(fen, "Ke4"),
    "bring king closer",
  );
  assert.equal(
  ...
  const middle16ExitFen = "k7/8/8/2K5/8/1B6/8/7N w - - 0 1";
  assert.equal(
    Brain.scoreKnightAndBishopWhiteMove(middle16ExitFen, "Kb6")
      .kingCloserOppositeBishopScore,
    99,
  );

  const diagonalProgressFen = "8/4k3/8/4B3/4K3/8/8/4N3 w - - 18 10";
  assert.equal(
    Brain.scoreKnightAndBishopWhiteMove(diagonalProgressFen, "Kd5")
      .kingCloserOppositeBishopScore,
    5,
  );
  assert.equal(
    Brain.scoreKnightAndBishopWhiteMove(diagonalProgressFen, "Kf5")
      .kingCloserOppositeBishopScore,
    5,
  );
  assert.deepEqual(Brain.getIdealEndgameWhiteMoves(diagonalProgressFen), [
    "Kd5",
    "Kf5",
  ]);
});
...
test("knight-bishop key-square pattern requires the bishop color", () => {
  setEndgame("knightAndBishop");

  const fen = "8/8/4B3/7k/4NK2/8/8/8 w - - 22 12";
  assert.equal(
    Brain.sameSquareColor("g4", Brain.findPiece(fen, "w", "b")!.square),
    true,
  );
  assert.equal(
    Brain.sameSquareColor("g5", Brain.findPiece(fen, "w", "b")!.square),
    false,
  );
  assert.equal(
    Brain.scoreKnightAndBishopWhiteMove(fen, "Kf5").keySquarePatternScore,
    2,
  );
});
...
test("knight-bishop rule 5 can establish the bishop zone x square", () => {
  ...
  assert.deepEqual(
    Brain.getKnightAndBishopZoneXSetup(Brain.getFen(fen, "Bf5")),
    {
      bishopSquare: "f5",
...
test("knight-bishop rule 8 prepares the bishop front square", () => {
  setEndgame("knightAndBishop");
...
test("knight-bishop rule 8 establishes the bishop in front of the white king", () => {
  ...
  assert.deepEqual(Brain.getIdealEndgameWhiteMoves(fen), ["Kd5", "Kf5"]);
  assert.equal(Brain.getEndgameReason(fen), "bring king closer");
  assert.equal(
...
test("knight-bishop rule 8 maintains the bishop in front of the white king", () => {
  ...
  const fen = "8/8/8/1k1N4/3BK3/8/8/8 w - - 14 8";
  assert.equal(
    Brain.getSquareInFrontOfWhiteKingBetweenKings("e4", "b5"),
    "d4",
  );
  assert.equal(
    Brain.scoreKnightAndBishopWhiteMove(fen, "Ne3").bishopInFrontScore,
    0,
  );
  assert.equal(
    Brain.getKnightAndBishopExplicitWhiteMoveReason(fen, "Ne3"),
    undefined,
  );
...
test("knight-bishop rule 9 keeps the knight closer to the white king before center", () => {
  ...
  assert.equal(
    Brain.scoreKnightAndBishopWhiteMove(fen, "Nb5+").knightWhiteKingDistance,
    4,
  );
  assert.equal(
  ...
  const whiteKingProximityFen =
    "8/8/4N3/4k3/6KB/8/8/8 w - - 8 5";
  assert.equal(
    Brain.scoreKnightAndBishopWhiteMove(whiteKingProximityFen, "Nf4")
      .knightWhiteKingDistance,
    1,
  );
  assert.equal(
    Brain.scoreKnightAndBishopWhiteMove(whiteKingProximityFen, "Nc5")
      .knightWhiteKingDistance,
    4,
  );
  assert.deepEqual(Brain.getIdealEndgameWhiteMoves(whiteKingProximityFen), [
    "Nf4",
  ]);
  assert.equal(
    Brain.getKnightAndBishopExplicitWhiteMoveReason(
      whiteKingProximityFen,
      "Nf4",
    ),
    "knight closer center",
  );
...
test("knight-bishop rule 9 keeps the knight behind the white king", () => {
  ...
  assert.equal(
    Brain.scoreKnightAndBishopWhiteMove(fen, "Ng3").knightBehindWhiteKingScore,
    0,
  );
  assert.equal(
...
test("two-bishop phase two takes direct opposition after edge-walk pressure", () => {
  setEndgame("twoBishops");
  ...
  assert.equal(directOpposition.phaseTwoTakeDirectOppositionPenalty, 0);
  assert.equal(movesBlackColorBishop.phaseTwoTakeDirectOppositionPenalty, 1);
...
test("two-bishop phase two can move the black-color bishop when checking into opposition", () => {
  ...
  assert.equal(checkingEdgeWalk.phaseTwoTakeDirectOppositionPenalty, 1);
...
test("two-bishop phase two pushes away from the controlled opposition edge square", () => {
  setEndgame("twoBishops");
  const fen = "1B6/8/2B5/k1K5/8/8/8/8 w - - 46 24";
  const awayFromControlledSquare = Brain.scoreTwoBishopsWhiteMove(fen, "Bb7");
  const towardControlledSquare = Brain.scoreTwoBishopsWhiteMove(fen, "Bc7+");
  ...
  assert.equal(awayFromControlledSquare.phaseTwoPushFromControlledEdgeSquarePenalty, 0);
  assert.equal(towardControlledSquare.phaseTwoPushFromControlledEdgeSquarePenalty, 1);
  assert.equal(Brain.compareTwoBishopsWhiteScores(awayFromControlledSquare, towardControlledSquare) < 0, true);
  assert.deepEqual(Brain.getIdealEndgameWhiteMoves(fen), ["Bb7"]);
  assert.equal(Brain.getEndgameReason(fen), "push from controlled edge square");
});
...
test("two-bishop phase two counts one-edge-step opposition pressure", () => {
  setEndgame("twoBishops");
  ...
  assert.equal(forceOpposition.phaseTwoTakeDirectOppositionPenalty, forceCorner.phaseTwoTakeDirectOppositionPenalty);
  ...
  assert.equal(forceOpposition.kingBishopScreeningPenalty > forceCorner.kingBishopScreeningPenalty, true);
});
...
test("two-bishop phase two prefers checking before bishop-corner distance", () => {
  ...
  assert.equal(Brain.getEndgamePhase(fen), "2/2");
  assert.equal(checking.phaseTwoForceOpponentCornerPenalty, 0);
  assert.equal(nonCheckingNearer.phaseTwoForceOpponentCornerPenalty, 0);
...
test("two-bishop phase two takes direct opposition before later phase-two details", () => {
  setEndgame("twoBishops");
  const fen = "8/8/8/1B6/1B6/8/k1K5/8 w - - 0 1";
  const forceCorner = Brain.scoreTwoBishopsWhiteMove(fen, "Kc3");
  ...
  assert.equal(forceCorner.phaseTwoTakeDirectOppositionPenalty, 1);
  assert.equal(takeOpposition.phaseTwoTakeDirectOppositionPenalty, 0);
...
test("two-bishop phase two uses corner distance after opposition", () => {
  ...
  assert.equal(towardCorner.phaseTwoTakeDirectOppositionPenalty, 0);
  assert.equal(centralBishops.phaseTwoTakeDirectOppositionPenalty, 0);
...
test("two-bishop phase two does not count distant edge-walk as opposition", () => {
  setEndgame("twoBishops");
  const fen = "8/8/8/8/8/1BBK4/8/2k5 w - - 18 10";
  const directOpposition = Brain.scoreTwoBishopsWhiteMove(fen, "Ba2");
  const distantEdgeWalk = Brain.scoreTwoBishopsWhiteMove(fen, "Bg8");

  assert.equal(Brain.getEndgamePhase(fen), "2/2");
  assert.equal(directOpposition.phaseTwoForceOpponentOppositionPenalty, 0);
  assert.equal(distantEdgeWalk.phaseTwoForceOpponentOppositionPenalty, 1);
  assert.equal(distantEdgeWalk.phaseTwoBishopCornerDistance > directOpposition.phaseTwoBishopCornerDistance, true);
  assert.equal(Brain.compareTwoBishopsWhiteScores(directOpposition, distantEdgeWalk) < 0, true);
  assert.deepEqual(Brain.getIdealEndgameWhiteMoves(fen), ["Ba2"]);
});

test("two-bishop phase two drives toward the current-edge corner near White's king", () => {
  setEndgame("twoBishops");
  const fen = "2k2B2/8/1K6/8/B7/8/8/8 w - - 26 14";
  const towardWhiteKingCorner = Brain.scoreTwoBishopsWhiteMove(fen, "Be7");
  const awayFromWhiteKingCorner = Brain.scoreTwoBishopsWhiteMove(fen, "Bd6");

  assert.equal(Brain.getEndgamePhase(fen), "2/2");
  assert.equal(towardWhiteKingCorner.phaseTwoForceOpponentCornerPenalty, 1);
  assert.equal(awayFromWhiteKingCorner.phaseTwoForceOpponentCornerPenalty, 3);
  assert.equal(Brain.compareTwoBishopsWhiteScores(towardWhiteKingCorner, awayFromWhiteKingCorner) < 0, true);
  assert.deepEqual(Brain.getIdealEndgameWhiteMoves(fen), ["Be7"]);
});
...
test("two-bishop phase two takes opposition before later priorities", () => {
  ...
  assert.equal(Brain.getEndgamePhase(fen), "2/2");
  assert.equal(opposition.phaseTwoTakeDirectOppositionPenalty, 0);
  assert.equal(middle.phaseTwoTakeDirectOppositionPenalty, 1);
...
test("two-bishop phase two takes the optimal opposition entry", () => {
  ...
  assert.equal(Brain.getEndgameReason(fen), "take direct opposition");
});
...
test("two-bishop white rules avoid king-bishop screening before king proximity", () => {
  setEndgame("twoBishops");
  const fen = "8/3k4/8/8/1BB5/2K5/8/8 w - - 8 5";
  const unscreened = Brain.scoreTwoBishopsWhiteMove(fen, "Kd4");
  const closerToBishops = Brain.scoreTwoBishopsWhiteMove(fen, "Kb3");

  assert.equal(unscreened.kingBishopScreeningPenalty, 0);
  assert.equal(closerToBishops.kingBishopScreeningPenalty, 2);
  assert.equal(unscreened.kingBishopDistance > closerToBishops.kingBishopDistance, true);
  assert.equal(Brain.compareTwoBishopsWhiteScores(unscreened, closerToBishops) < 0, true);
  assert.deepEqual(Brain.getIdealEndgameWhiteMoves(fen), ["Kd4"]);
  assert.equal(Brain.getEndgameReason(fen), "avoid king bishop screening");
});

test("two-bishop white rules prefer king near bishops before bishops closer", () => {
  setEndgame("twoBishops");
  const fen = "8/3k4/8/8/1BB5/2K5/8/8 w - - 8 5";
  const kingNearer = Brain.scoreTwoBishopsWhiteMove(fen, "Ba5");
  const bishopsCloser = Brain.scoreTwoBishopsWhiteMove(fen, "Bf7");

  assert.equal(kingNearer.kingBishopDistance < bishopsCloser.kingBishopDistance, true);
  assert.equal(
    kingNearer.bishopBlackKingDistance > bishopsCloser.bishopBlackKingDistance,
    true,
  );
  assert.equal(
    Brain.compareTwoBishopsWhiteScores(kingNearer, bishopsCloser) < 0,
    true,
  );
});

test("two-bishop white rules use bishop distance after king-bishop distance", () => {
  setEndgame("twoBishops");
  const fen = "8/3k4/8/8/1BB5/2K5/8/8 w - - 8 5";
  const closer = Brain.scoreTwoBishopsWhiteMove(fen, "Ba5");
  const farther = Brain.scoreTwoBishopsWhiteMove(fen, "Ba3");

  assert.equal(closer.kingBishopDistance, farther.kingBishopDistance);
  assert.equal(closer.kingBishopScreeningPenalty, farther.kingBishopScreeningPenalty);
  assert.equal(closer.bishopAdjacencyPenalty, farther.bishopAdjacencyPenalty);
  assert.equal(closer.bishopBlackKingDistance, 6);
  assert.equal(farther.bishopBlackKingDistance, 7);
  assert.equal(Brain.compareTwoBishopsWhiteScores(closer, farther) < 0, true);
});

test("two-bishop king near bishops uses total king walk distance to bishops", () => {
  setEndgame("twoBishops");

  assert.equal(
    Brain.scoreTwoBishopsWhiteMove(
      "8/3k4/8/8/1BB5/2K5/8/8 w - - 8 5",
      "Kd4",
    ).kingBishopDistance,
    3,
  );
...
test("endgame priority help does not hide active queen and rook rules", () => {
  ...
  assert.equal(
    knightAndBishopHelp.whitePriorities.includes(
      "[prepare] Reach the knight's key-square pattern or force Black into Zone X when available. Establish the bishop on its Zone X square when available. Prepare * is true when the bishop is on its Zone X square: move White's king toward Black's king, otherwise move the knight by the shortest path to its Zone X square.",
    ),
    true,
  );
  assert.equal(
...
test("two-bishop priority help explains phase-two terms concretely", () => {
  ...
  assert.match(text, /take direct king opposition, unless it moves the white king into a square controlled by a bishop/);
  assert.match(text, /edge square two squares from Black's king and diagonally two squares from White's king/);
  assert.match(text, /force Black towards the corner along its current edge and closer to White's king/);
  assert.match(text, /Enter or remain in phase 2/);
  ...
  assert.match(text, /Keep White's king and bishops from screening each other from Black's king/);
  assert.match(text, /Keep White's king near the bishops/);
  assert.match(text, /Force Black to the edge/);
  assert.doesNotMatch(text, /Take king opposition if king is already adjacent to both bishops/);
  assert.doesNotMatch(text, /force Black to stay in phase 2/);
  assert.doesNotMatch(text, /keep Black's king on the edge or in a corner/);
  assert.equal(
    help.whitePriorities.indexOf("Enter or remain in phase 2.") <
